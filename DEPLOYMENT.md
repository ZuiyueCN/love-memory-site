...
3. 当前 V1 会在浏览器端把照片统一转成普通 JPG，避免 HDR 照片导致页面闪烁；单张图片保存前限制为 4MB。阿里云服务器部署时建议把 Nginx `client_max_body_size` 设为 `50M`，方便组图上传。
4. 确认本地生产构建通过：
...
- 还没配置 HTTPS、只是用 `http://公网IP` 访问时，`AUTH_COOKIE_SECURE` 必须填 `false`，否则登录 cookie 不会正常发送。
- 背景音乐默认读取 `public/music/background.mp3`。如果换成外部音乐地址，可以设置 `NEXT_PUBLIC_BACKGROUND_MUSIC_URL`。
